...
export const FETCH_RANGE_REQUEST = `${prefix}/FETCH_RANGE_REQUEST`
export const FETCH_RANGE_START = `${prefix}/FETCH_RANGE_START`
export const FETCH_RANGE_SUCCESS = `${prefix}/FETCH_RANGE_SUCCESS`
...
export function fetchRangeOfEvents() {
    return {
        type: FETCH_RANGE_REQUEST
    }
}
...
export function* fetchRangeSaga() {
    ...
    const last = entities.last() ? entities.last().uid : '';
	const ref = firebase.database().ref('events').orderByKey().limitToFirst(10).startAt(last)
...
export function * saga() {
    yield all([
        ...
        takeEvery(FETCH_RANGE_REQUEST, fetchRangeSaga)
    ])
}